...
class GUIPanelPOI {
    ...
    setContent(content) {
        this.objUI.getChildByName('title').getComponent(UITextField).text = content.title
        this.objUI.getChildByName('body').getComponent(UITextField).text = content.body
    }
...
class Sample_UIMultiPanel {
    ...
    async run() {
        ...
        await Engine3D.init({
            canvasConfig:{
                canvas: document.getElementById("canvas"), // Specify the canvas element, you can customize the canvas size or layout
